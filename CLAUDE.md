...
- [telestrator/history.as](telestrator/history.as) — `UndoLast` / `RedoLast` / `ClearRedoStack` / `ClearAll` / `DeleteSelected` / `ForgetHistoryFor`. Operates on the `g_Drawables`, `g_UndoStack`, and `g_RedoStack` globals declared in main.as.
...
- Mutations to `g_Drawables` go through `StartStroke` / `AppendPointToActiveStroke` / `FinishStroke` for streaming pen/highlighter strokes, or `CommitPending` for press-drag-release shapes (and the deferred TextLabel popup, which lives in `g_Pending` while the user is typing). The Marker tool inserts directly. `UndoLast` / `RedoLast` / `ClearAll` / `DeleteSelected` are the only paths that pop or wipe; the eraser and fade-prune also remove but skip the undo stack via `ForgetHistoryFor`.
- Undo/redo runs off an explicit operation stack. Every committed creation or deletion pushes a `HistoryOp` (`HOP_Create` / `HOP_Delete`) onto `g_UndoStack`; `UndoLast` pops one and reverses it, moving it to `g_RedoStack`. `HOP_Delete` records the original index so a re-insert restores z-order. The eraser and `PruneFaded` call `ForgetHistoryFor(d)` after a silent removal to drop matching ops from both stacks — without that, a later `UndoLast` would try to operate on a target that's already gone. `LoadState` seeds `g_UndoStack` with one `HOP_Create` per loaded drawable so reloaded marks stay undoable; the stacks themselves are not persisted.
- After any committed mutation, call `ClearRedoStack()` *before* `SaveState()` — a new branch in undo history invalidates the redo stack. `UndoLast` and `RedoLast` are the two exceptions; they shuttle ops between `g_UndoStack` and `g_RedoStack`.
